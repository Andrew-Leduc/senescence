...
req.pkg<-c("plyr", "gridExtra", "stringr", "ggpubr", "Rcpp", "reshape2",
           ...
           "matrixStats","patchwork", "scales","stylo", "readr", "tidyverse","rlang","vctrs","tibble","dplyr","dbplyr")
...
Population_GSEA <- function(data,GO_db,sn,mouse,converter_df){
  ...
  for (i in 1:length(unique_GO_terms)){
    ...
    if(mouse == 1){
      mouse_prots <- converter_df %>% filter(human_uni %in% GO_db_lim$Uniprot)
      data_matches <- data %>% dplyr::filter(Protein %in% mouse_prots$mouse_uni)
    }
    else{
